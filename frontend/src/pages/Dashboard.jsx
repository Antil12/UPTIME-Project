...
const Background = () => (
  <div className="pointer-events-none fixed inset-0 z-0 overflow-hidden">
    ...
    <div className="absolute inset-0" style={{ background: "radial-gradient(ellipse 70% 45% at 50% 0%, rgba(56,189,248,0.05) 0%, transparent 100%)" }} />
    <div className="absolute" style={{ top: "60%", left: "10%", width: 340, height: 340, background: "radial-gradient(circle, rgba(129,140,248,0.035) 0%, transparent 68%)", filter: "blur(90px)" }} />
    <div className="absolute" style={{ top: "15%", right: "8%", width: 260, height: 260, background: "radial-gradient(circle, rgba(16,185,129,0.028) 0%, transparent 68%)", filter: "blur(85px)" }} />
    <div className="absolute" style={{ bottom: "5%", right: "20%", width: 240, height: 240, background: "radial-gradient(circle, rgba(56,189,248,0.022) 0%, transparent 68%)", filter: "blur(75px)" }} />
    <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(rgba(148,163,184,0.022) 1px, transparent 1px), linear-gradient(90deg, rgba(148,163,184,0.022) 1px, transparent 1px)", backgroundSize: "42px 42px" }} />
    <motion.div
...
const HUDCorner = ({ pos, delay = 0 }) => {
  ...
  return (
    ...
    >
      <motion.div className="absolute top-0 left-0 h-[1.5px] bg-sky-400/65" initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ delay: delay + 0.15, duration: 0.35 }} />
      <motion.div className="absolute top-0 left-0 w-[1.5px] bg-sky-400/65" initial={{ height: 0 }} animate={{ height: "100%" }} transition={{ delay: delay + 0.15, duration: 0.35 }} />
    </motion.div>
  );
};
...
const StatusDot = ({ color = "#34d399", label }) => (
  <div className="flex items-center gap-2">
    ...
    {label && (
      <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", letterSpacing: "0.15em", color: `${color}88`, textTransform: "uppercase" }}>
        {label}
      </span>
    )}
  </div>
  ...
  >
    ...
    <div className="relative z-10">
      <div className="flex items-center justify-between mb-3">
        <div className="w-9 h-9 rounded-xl flex items-center justify-center border" style={{ borderColor: `${accentColor}18`, background: `${accentColor}07` }}>
          <Icon size={15} style={{ color: accentColor }} />
        </div>
        <StatusDot color={accentColor} />
      </div>
      <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", color: "rgba(148,163,184,0.48)", letterSpacing: "0.13em", textTransform: "uppercase" }}>{label}</div>
      <div className="mt-2" style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 700, fontSize: "23px", letterSpacing: "0.03em", color: "white", textShadow: `0 0 16px ${accentColor}18` }}>{value}</div>
      {sublabel && <div className="mt-1" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", color: `${accentColor}55`, letterSpacing: "0.08em" }}>{sublabel}</div>}
    </div>
  </motion.div>
);
...
const LiveTicker = ({ urls }) => {
  ...
  const items = [
    ...downSites.map((s) => ({ label: s.domain, type: "DOWN", color: "#f87171" })),
    ...slowSites.map((s) => ({ label: s.domain, type: "SLOW", color: "#fbbf24" })),
    { label: `${urls.filter((u) => u.status === "UP").length} sites nominal`, type: "OK", color: "#34d399" },
  ];
  return (
    <div className="relative overflow-hidden rounded-xl py-2 px-4" style={{ background: "rgba(3,7,18,0.58)", border: "1px solid rgba(56,189,248,0.07)", backdropFilter: "blur(10px)" }}>
      <div className="flex items-center gap-3 overflow-hidden">
        <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", letterSpacing: "0.18em", color: "rgba(56,189,248,0.45)", textTransform: "uppercase", whiteSpace: "nowrap" }}>LIVE ●</span>
        <div className="flex gap-6 overflow-hidden" style={{ maskImage: "linear-gradient(90deg, transparent, black 8%, black 92%, transparent)" }}>
          <motion.div className="flex gap-6 whitespace-nowrap" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 20, repeat: Infinity, ease: "linear" }}>
            {[...items, ...items].map((item, i) => (
              <span key={i} style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", color: item.color, letterSpacing: "0.08em" }}>
                <span style={{ opacity: 0.5 }}>◆</span> {item.label} <span style={{ opacity: 0.35 }}>[{item.type}]</span>
              </span>
            ))}
          </motion.div>
        </div>
      ...
      <main className="relative z-10 px-4 sm:px-6 lg:px-8 py-5 max-w-[1500px] mx-auto space-y-4">
        ...
        >
          <div>
            <div className="flex items-center gap-3 mb-2">
              <div className="h-[1px] w-7" style={{ background: "rgba(56,189,248,0.25)" }} />
              <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", letterSpacing: "0.28em", color: "rgba(56,189,248,0.45)", textTransform: "uppercase" }}>
                Uptime Command Center
              </span>
              <div className="h-[1px] w-20" style={{ background: "rgba(56,189,248,0.1)" }} />
            </div>
            <h1 style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 800, fontSize: "clamp(22px, 4vw, 30px)", letterSpacing: "0.05em", color: "white", textShadow: "0 0 28px rgba(56,189,248,0.08)" }}>
              DASHBOARD
            </h1>
            <div className="mt-2" style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "9px", color: "rgba(148,163,184,0.4)", letterSpacing: "0.05em" }}>
          ...
          <div className="flex items-center gap-4">
            {downSites.length > 0 && (
              ...
              >
                <motion.div className="w-1.5 h-1.5 rounded-full bg-red-400" animate={{ opacity: [1, 0.3, 1] }} transition={{ duration: 0.8, repeat: Infinity }} />
                <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "8px", color: "#f87171", letterSpacing: "0.12em", textTransform: "uppercase" }}>
        ...
        <section className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <HudStatCard
            ...
            value={urls.length} accentColor="#38bdf8"
        ...
        <motion.div
          ...
          style={{ background: "rgba(3,7,18,0.66)", border: "1px solid rgba(56,189,248,0.08)", backdropFilter: "blur(14px)", boxShadow: "0 0 18px rgba(56,189,248,0.02)" }}
        >
          <div className="flex items-center gap-4">
            <div className="h-7 w-[2px] rounded-full" style={{ background: "linear-gradient(to bottom, rgba(56,189,248,0.6), transparent)" }} />
            <div>
              <div style={{ fontFamily: "'Orbitron', sans-serif", fontWeight: 700, fontSize: "10px", letterSpacing: "0.08em", color: "white" }}>SITE MONITORING PANEL</div>
              <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "9px", color: "rgba(148,163,184,0.45)", marginTop: "3px" }}>
                {filtersActive
        ...
        >
          ...
          <div className="relative w-full max-w-sm group">
            ...
            <input
              ...
              style={{ background: "rgba(255,255,255,0.025)", border: "1px solid rgba(56,189,248,0.09)", color: "white", fontFamily: "'JetBrains Mono', monospace", fontSize: "11px", letterSpacing: "0.02em", backdropFilter: "blur(12px)" }}
              onFocus={(e) => { e.target.style.border = "1px solid rgba(56,189,248,0.34)"; e.target.style.boxShadow = "0 0 0 3px rgba(56,189,248,0.06), 0 0 18px rgba(56,189,248,0.035)"; }}
        ...
        <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.32, duration: 0.5 }}>
          {tableRows.length === 0 ? (
            <div className="rounded-2xl p-12 text-center" style={{ background: "rgba(3,7,18,0.68)", border: "1px solid rgba(56,189,248,0.08)", backdropFilter: "blur(16px)" }}>
              ...
              <h3 className="text-white mb-2" style={{ fontFamily: "'Orbitron', sans-serif", letterSpacing: "0.05em", fontWeight: 700, fontSize: "14px" }}>NO SITES FOUND</h3>
        ...
        {!filtersActive && totalPages > 1 && (
          <div className="mt-4 flex items-center justify-center gap-3">
            <button
              ...
              style={{ background: "rgba(255,255,255,0.03)", color: "rgba(148,163,184,0.7)", fontFamily: "'JetBrains Mono', monospace", fontSize: "11px" }}
            >
              Prev
            </button>
            <div style={{ color: "rgba(148,163,184,0.8)", fontSize: 13, fontFamily: "'JetBrains Mono', monospace" }}>
